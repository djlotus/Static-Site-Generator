if (document.querySelector("[data-tab]")) {
   ...
   var currentTab, // create variable for currently active tab
      ...
      control =
         ...
         document.getElementById("accordions"); // get tabs container
   ...
   function setPreviousSiblings(el, att, state) {
      ...
      if (document.previousElementSibling) {
         // if broswer supports previousElementSibling
         while ((el = el.previousElementSibling.nodeName == "LI")) {
            // while current element has a previous element type "LI"
            siblings.push(el); // push current previous element to array
         }
      } else {
         // if sad browser :(
         while ((el = el.previousSibling)) {
            // while current element has a previous sibling (anytype)
            if (el.nodeType === 1 && el.nodeName == "LI") {
               // once that sibling is element type "LI"
               siblings.push(el); // push current previous element to array
            }
         }
      }
   ...
   function setNextSiblings(el, att, state) {
      ...
      if (document.nextElementSibling) {
         while ((el = el.nextElementSibling.nodeName == "LI")) {
            // while current element has a next element type "LI"
            siblings.push(el); // push current next element to array
         }
      } else {
         // if sad browser :(
         while ((el = el.nextSibling)) {
            // while current element has a next sibling
            if (el.nodeType === 1 && el.nodeName == "LI") {
               // if that sibling is element type "LI"
               siblings.push(el); // push current next element to array
            }
         }
      }
      ...
      for (var i = 0; i <= siblings.length - 1; i++) {
         // for each element in array "siblings"
         siblings[i].setAttribute(att, state); // set attribute to "passive"
      }
   ...
   control.addEventListener("click", function (e) {
      // set event on "click" of control element
      var target = e.target,
      ...
      if (target && tLi) {
         ...
         if (target.getAttribute(attrTab) === "active") {
            target.setAttribute(attrTab, ""); // add value "active" to attribute
            target.setAttribute("aria-selected", "false"); // pass state to AT
            target.setAttribute("class", "fa-solid icon-plus"); // set class to show expanded icon
            resetPanels();
         } else {
            target.setAttribute(attrTab, "active"); // add value "active" to attribute
            target.setAttribute("aria-selected", "true"); // pass state to AT
            target.setAttribute("class", "fa-solid icon-minus"); // set class to show expanded icon
            ...
            setPreviousSiblings(target, "class", "fa-solid icon-plus"); // set class to show collapsed icon

            setNextSiblings(target, attrTab, ""); // set next elements to ""
            setNextSiblings(target, "aria-selected", "false"); // pass state to AT
            setNextSiblings(target, "class", "fa-solid icon-plus"); // set class to show collapsed icon
            // reset panels
            resetPanels();
   ...
   control.addEventListener("keydown", function (e) {
      ...
      if (e.code == "ArrowRight") {
         ...
         if (nextTab != null) {
            ...
            currentTab.setAttribute("class", "fa-solid icon-plus"); // set class to show collapsed icon

            nextTab.setAttribute(attrTab, "active"); // set next elements to ""
            nextTab.setAttribute("aria-selected", "true"); // expose state to AT
            nextTab.setAttribute("class", "fa-solid icon-minus"); // set class to show expanded icon
      ...
      if (e.code == "ArrowLeft") {
         ...
         if (prevTab != null) {
            ...
            prevTab.setAttribute("class", "fa-solid icon-minus"); // set class to show expanded icon
